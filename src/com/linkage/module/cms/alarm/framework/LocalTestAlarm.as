package com.linkage.module.cms.alarm.framework
{
	...
	public class LocalTestAlarm
	{
		...
		public function LocalTestAlarm(alarmServer:AlarmServer)
		{
			//发送故障告警
			sendFaultAlarm(alarmServer, 3);

			//发送移除告警
			setTimeout(function():void
				{
				//sendRemoveAlarm(alarmServer);
				}, 61000);
		...
		//发送移除告警
		private function sendRemoveAlarm(alarmServer:AlarmServer, sendNum:int = -1):void
		{
			var countRM:int = 0;
			var key:uint = setInterval(function():void
				{
					if (sendNum > 0 && countRM == sendNum)
					{
						clearInterval(key);
						return;
					}
					...
					stat["nac"] = alarmnac;
					stat["msg_topic"] = "as";
					var faultArray:Array = getRemoveAlarm(countRM);
					faultArray.push(stat);
					fault["91"] = faultArray;
					alarmServer.handlerAlarm(fault);
					countRM++;
		...
		//发送告警风暴
		private function sendAlarmStorm(alarmServer:AlarmServer):void
		{
			//告警风暴开始
			setTimeout(function():void
				{
					var alarmStorm:Object = {AlarmStorm: 'start'};
					alarmServer.handlerAlarm(alarmStorm);
				}, 1000);

			//告警风暴结束
			setTimeout(function():void
				{
					var alarmStorm:Object = {AlarmStorm: 'stop'};
					alarmServer.handlerAlarm(alarmStorm);
				}, 10000);
		}
