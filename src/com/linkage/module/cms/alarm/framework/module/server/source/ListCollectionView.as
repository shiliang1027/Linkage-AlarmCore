...
package com.linkage.module.cms.alarm.framework.module.server.source
{
	...
	public class ListCollectionView extends Proxy implements ICollectionView, IList, IMXMLObject
	{
		...
		//
		//--------------------------------------------------------------------------
		private var log:ILogger = Log.getLogger("com.linkage.module.cms.alarm.framework.module.server.source.ListCollectionView");
		/**
		 *  @private
		...
		override flash_proxy function getProperty(name:*):*
		{
			...
			if (index == -1)
			{
				var message:String = resourceManager.getString("collections", "unknownProperty", [name]);
				...
				for (var i:int = 0; i < items.length; i++)
				{
					...
					if (filterFunction == null || filterFunction(item))
					{
						...
						if (sort && sort.unique && sort.compareFunction(item, localIndex[loc]) == 0)
						{
							...
							var message:String = resourceManager.getString("collections", "incorrectAddition");
		...
		mx_internal function findItem(values:Object, mode:String, insertIndex:Boolean = false):int
		{
			if (!sort || !localIndex)
			{
				var message:String = resourceManager.getString("collections", "itemNotFound");
				throw new CollectionViewError(message);
			}
		...
		mx_internal function getBookmark(index:int):ListCollectionViewBookmark
		{
			if (index < 0 || index > length)
			{
				var message:String = resourceManager.getString("collections", "invalidIndex", [index]);
				throw new CollectionViewError(message);
			}
		...
		mx_internal function getBookmarkIndex(bookmark:CursorBookmark):int
		{
			...
			{
				var message:String = resourceManager.getString("collections", "bookmarkNotFound");
				throw new CollectionViewError(message);
			}
		...
		private var _numberCols:Array; //数字列，凡是在此范围的列，均按照数字类型排序
		private var _groupCols:Array; //分组列，凡是在此范围内的列，
		private var _defaultSort:Sort; //默认的排序列
		...
		private function internalRefresh(dispatch:Boolean):Boolean
		{
			...
			if (sort || filterFunction != null)
			{
				...
				if (sort)
				{
					doLog("sort start");
					//锁定排序
					var lockArray:Array = []; //锁定的数据集合
					var freeArray:Array = []; //未锁定的数据集合
					for each (var obj:Object in localIndex)
					{
						if (obj._locked && obj._locked == 1)
						{
							lockArray.push(obj);
						}
						else
					...
					localIndex = freeArray;
					//分离锁定的数据 
					if (sort.fields)
					{
						if (numberCols && numberCols.length > 0)
						{
							for each (var sf:SortField in sort.fields)
							{
								if (numberCols.indexOf(sf.name) != -1)
								{
									doLog("数字排序字段" + sf.name);
									sf.numeric = true;
								}
							}
						}
						//是否执行默认排序
						var ds:Boolean = defaultSort != null;
						if (ds)
						{
							for each (var cof:SortField in sort.fields)
							{
								if (cof.name == (defaultSort.fields[0] as SortField).name)
								{
									doLog("不需要默认排序");
									ds = false;
									break;
						...
						if (sort.fields.length == 1 && groupCols && groupCols.indexOf((sort.fields[0] as SortField).name) != -1)
						{
							doLog("分组排序" + (sort.fields[0] as SortField).descending + "," + (sort.fields[0] as SortField).name);
							var tmpdic:Dictionary = new Dictionary();
							var tmpkey:Array = [];
							var tmpArray:Array = [];
							var sn:String = (sort.fields[0] as SortField).name;
							for each (var ob:Object in localIndex)
							{
								var key:Object = ob[sn];
								if (tmpdic[key] != null)
								{
									(tmpdic[key] as Array).push(ob);
								}
								else
								{
									tmpdic[key] = [ob];
							...
							localIndex = [];
							var oldsfs:Array = sort.fields;
							var tmpsf:SortField = new SortField();
							tmpsf.descending = (sort.fields[0] as SortField).descending;
							tmpsf.numeric = (sort.fields[0] as SortField).numeric;
							sort.fields = [tmpsf];
							sort.sort(tmpkey);

							if (ds)
							{
								for each (var idx:Object in tmpkey)
								{
									doLog("默认分组排序key:" + idx);
									defaultSort.sort((tmpdic[idx] as Array));
									tmpArray = tmpArray.concat((tmpdic[idx] as Array));
								}
							}
							else
							{
								for each (var idx1:Object in tmpkey)
								{
									doLog("无默认排序key:" + idx);
									tmpArray = tmpArray.concat((tmpdic[idx] as Array));
								}
							}
							localIndex = localIndex.concat(tmpArray);
							//排序锁定的数组，然后拼接到最前边
							if (lockArray.length != 0)
							{
								...
								tmpkey.length = 0;
								tmpArray = [];
								for each (var ob1:Object in lockArray)
								{
									var key1:Object = ob1[sn];
									if (tmpdic[key1] != null)
									{
										(tmpdic[key1] as Array).push(ob1);
									}
									else
									{
										tmpdic[key1] = [ob1];
								...
								if (ds)
								{
									for each (var idx2:Object in tmpkey)
									{
										doLog("默认分组排序key:" + idx2);
										defaultSort.sort((tmpdic[idx2] as Array));
										tmpArray = tmpArray.concat((tmpdic[idx2] as Array));
									}
								}
								else
								{
									for each (var idx3:Object in tmpkey)
									{
										doLog("无默认排序key:" + idx3);
										tmpArray = tmpArray.concat((tmpdic[idx3] as Array));
									}
								}
								tmpArray = tmpArray.concat(localIndex);
								localIndex.length = 0;
								localIndex = localIndex.concat(tmpArray);
							}
							sort.fields = oldsfs; //还原排序信息
							tmpArray.length = 0;
							tmpdic = null;
							tmpkey.length = 0;
							tmpsf = null;
						...
						else
						{
							if (ds)
							{
								sort.fields.push(defaultSort.fields[0]);
								doLog("默认 普通排序");
								sort.sort(localIndex);
								if (lockArray.length != 0)
								{
									sort.sort(lockArray);
									localIndex = lockArray.concat(localIndex);
									lockArray.length = 0;
								}
								sort.fields.pop();
							}
			...
			//log.warn("refresh sort time:" + (endTime - startTime));
			revision++;
			pendingUpdates = null;
			if (dispatch)
			{
				...
				dispatchEvent(refreshEvent);
					...
					//log.warn("refresh view time:" + (new Date().getTime() - endTime));
			}
			return true;
		}

		// 打印日志
		private function doLog(info:*):void
		{
//			MonsterDebugger.trace("ListCollectionView", info);
		}
		...
		public function get numberCols():Array
		{
			return _numberCols;
		}

		public function set numberCols(value:Array):void
		{
			_numberCols = value;
		}

		public function get groupCols():Array
		{
			return _groupCols;
		}

		public function set groupCols(value:Array):void
		{
			_groupCols = value;
		}

		public function get defaultSort():Sort
		{
			return _defaultSort;
		}

		public function set defaultSort(value:Sort):void
		{
			_defaultSort = value;
			if (!sort)
			{
...
class ListCollectionViewCursor extends EventDispatcher implements IViewCursor
{
	 ...
	 *  @private
	 */
	private var _view:com.linkage.module.cms.alarm.framework.module.server.source.ListCollectionView;
	...
	public function insert(item:Object):void
	{
		...
		else if (beforeFirst)
		{
			if (view.length > 0)
			{
				var message:String = resourceManager.getString("collections", "invalidInsert");
	...
	public function remove():Object
	{
		...
		{
			var message:String = resourceManager.getString("collections", "invalidRemove");
			throw new CursorError(message);
		}
	...
	private function checkValid():void
	{
		if (invalid)
		{
			var message:String = resourceManager.getString("collections", "invalidCursor");
			throw new CursorError(message);
		}
	}
...
class ListCollectionViewBookmark extends CursorBookmark
{
	...
	mx_internal var view:com.linkage.module.cms.alarm.framework.module.server.source.ListCollectionView;
